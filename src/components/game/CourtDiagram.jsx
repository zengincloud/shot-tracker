...
export default function CourtDiagram() {
  ...
  return (
    <div className={`court-diagram ${isZoneStep ? 'active-step' : ''}`}>
      ...
      <svg viewBox="0 0 300 200" className="court-svg">
        {/* Court outline */}
        <rect x="5" y="5" width="290" height="190" rx="2" fill="none" stroke="var(--border)" strokeWidth="2" />

        {/* Paint */}
        <rect x="100" y="100" width="100" height="95" fill="none" stroke="var(--border)" strokeWidth="1.5" />

        {/* Free throw circle */}
        <circle cx="150" cy="100" r="30" fill="none" stroke="var(--border)" strokeWidth="1" strokeDasharray="4 4" />

        {/* 3-point arc */}
        <path
          d="M 30 195 L 30 80 Q 150 -20 270 80 L 270 195"
          fill="none"
          stroke="var(--border)"
          strokeWidth="1.5"
        />
        ...
        {/* Basket */}
        <circle cx="150" cy="185" r="5" fill="none" stroke="var(--text-muted)" strokeWidth="1.5" />

        {/* Zone hotspots */}
        {ZONES.map(zone => {
          const cx = (zone.x / 100) * 290 + 5;
          const cy = (zone.y / 100) * 190 + 5;
